...
export default function ConfirmEmailPage(){
    ...
    return<div className={styles.confirmEmailPanel}>
                <Loading isLoading={isLoading} isSuccessful={isSuccessful}/>
                <header className={styles.h2}>{text}</header>
                <div className={styles.flexRow}>
                    {!isSuccessful &&
                        <button className={styles.button} onClick={onTryAgainButtonClicked}>Spróbuj ponownie</button>
                    }
                    <button className={styles.button} onClick={onGoBackButtonClicked}>Wróć do ekranu logowania</button>
                </div>
    </div>
